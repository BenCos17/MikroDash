...
function resetDeviceView() {
  ...
  if (routerTag) routerTag.textContent = '';
  if (liveRx) liveRx.textContent = '\u2014';
  if (liveTx) liveTx.textContent = '\u2014';
  if (wanIpDisplay) wanIpDisplay.textContent = '\u2014';
  if (connTotal) connTotal.textContent = '\u2014';
}

function renderDeviceOptions(payload) {
  if (!deviceSelect || !payload || !payload.devices) return;
  knownDevices = payload.devices.slice();
  ...
  if (payload.defaultDeviceId) {
    deviceSelect.value = payload.defaultDeviceId;
  }
}
...
function makeChartObj() {
  ...
  chart = new Chart(trafficCtx, {
    type: 'line', data: {
      ...
      scales: {
        x: { display: true, grid: { color: 'rgba(99,130,190,.07)' }, ticks: { color: 'rgba(148,163,190,.4)', font: { family: "'JetBrains Mono',monospace", size: 10 }, maxTicksLimit: 8, maxRotation: 0 } },
...
socket.on('ifstatus:update', function (data) {
  ...
  ifaceGrid.innerHTML = ifaces.map(function (i) {
    ...
    return '<div class="iface-tile ' + cls + '">' +
      ...
      (poeStr ? '<div class="iface-poe">' + poeStr + '</div>' : '') +
      '</div>';
  }).join('');
});
...
// ── Device selection ──────────────────────────────────────────────────────
socket.on('devices:list', function (payload) {
  renderDeviceOptions(payload || {});
});

socket.on('device:selected', function (payload) {
  currentDeviceId = payload && payload.deviceId ? payload.deviceId : '';
  if (deviceSelect && currentDeviceId) deviceSelect.value = currentDeviceId;
  resetDeviceView();
});

if (deviceSelect) {
  deviceSelect.addEventListener('change', function () {
    var newDeviceId = deviceSelect.value;
    if (!newDeviceId || newDeviceId === currentDeviceId) return;
    socket.emit('device:select', { deviceId: newDeviceId });
  });
}

// ── Reconnect ──────────────────────────────────────────────────────────────
socket.on('disconnect', function () { reconnectBanner.classList.add('show'); });
socket.on('connect', function () { reconnectBanner.classList.remove('show'); resetDeviceView(); });
  ...
  return new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { display: false },
        y: {
          display: true, min: 0, grid: { color: 'rgba(99,130,190,.08)' },
...
(function () {
  ...
  socket.on('conn:update', function (data) {
    ...
    var ccUrl = currentDeviceId ? ('/api/localcc?deviceId=' + encodeURIComponent(currentDeviceId)) : '/api/localcc';
    fetch(ccUrl).then(function (r) { return r.json(); }).then(function (d) {
